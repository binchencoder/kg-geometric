...
=====================================================================
统一推理 —— HTTP 服务（静态链接预测 + 时序属性预测 + TLP 链接预测）

基于 Python 标准库 http.server 实现，零额外依赖（无需 FastAPI/Flask）。
启动时按需加载三类模型，随后通过 REST 接口对外提供：

  A. 静态链接预测（src/slp/predict.py）
     输入症状文本（换行分隔），复用 SLPModelHandler 的四阶段推理
     （语义匹配 → 故障定位 → 答案生成 → 结果组装），输出含 causes /
     actions / tools 的结构化静态链接预测 JSON。

  B. 时序属性预测（src/tap/predict.py）
     加载训练时持久化的异构图与 R-GCN / TGN 权重，对指定时序切片输出
     健康状态、油温趋势与故障风险等级。

  C. TLP 时序链接预测（src/tlp/predict.py）
...
1) 健康检查
    GET  /health
    -> {"status": "ok", "device": ..., "slp_models": [...],
        "tap_config_loaded": true, "tlp_config_loaded": true}
...
2) 静态链接预测（JSON 请求体）
    POST /slp
    {
      ...
      "top_k":    3,                      # 可选，默认服务端配置
      "model":    "slp_model_xxx"   # 可选，默认首个发现的模型
    }
    -> 结构化静态链接预测 JSON（diagnosis / top_fault / causes / actions / tools ...）
...
说明
-------------------------------------------------------------------
* 静态链接预测模型自动发现 models 目录下 .pt/.pth 文件并逐一加载缓存；
  时序属性预测（TAP）需要 --tap-model-dir 目录下存在 temporal_kg_graph.pt、
  temporal_diag_rgcn.pth、temporal_tgn.pth；
  TLP 需要 --model-path 指向已训练的 .pt checkpoint。
...
from src.slp.predict import (  # noqa: E402
    SLPModelHandler,
    _discover_models,
    register_handler,
)

from src.tap.predict import (  # noqa: E402
    RGCNFaultDiagnosis,
...
from src.tlp.predict import (  # noqa: E402
    predict_tails,
    _resolve_id,
    evaluate_filtered,
...
# 默认静态链接预测配置（来自 config.yaml 的 inference.slp_prediction）
def _slp_config() -> SLPConfig:
    try:
        from src.core.config import InferenceConfig
        return InferenceConfig.default().slp_config
    except Exception:  # noqa: BLE001
        return {}


# 默认时序属性预测配置（来自 config.yaml 的 inference.tap_prediction）
def _tap_config() -> TAPConfig:
    try:
        from src.core.config import InferenceConfig
        return InferenceConfig.default().tap_config
    except Exception:  # noqa: BLE001
        return {}


def _tlp_config() -> TLPConfig:
    try:
        from src.core.config import InferenceConfig
...
class InferenceService:
    ...
    线程安全：模型参数只读；静态链接预测与趋势推理均在 ``torch.no_grad()`` 下执行，
    并加锁保护共享模型的并发前向传播。
    """
    ...
    def __init__(
            ...
            slp_model_dir: str,
            tap_model_dir: str,
            tlp_model_path: "str | None" = None,
            device: str = "cpu",
            top_k: int = 3,
            relation_mapping: "dict | None" = None,
    ):
        self.device = device
        self.default_top_k = top_k
        self.relation_mapping = relation_mapping or _default_relation_mapping()
        self._lock = threading.Lock()

        # 静态链接预测模型缓存: name -> (model, meta, data, dataset)
        self._slp_cache: dict = {}
        # 时序属性预测（TAP）资源
        self._tap_config: "dict | None" = None
        # TLP 链接预测资源
        self._tlp_config: "dict | None" = None

        # 注册静态链接预测模型处理器，使 _discover_models 能按文件名前缀自动匹配
        register_handler(
            "slp_model",
            SLPModelHandler(
                device=self.device,
        ...
        self._load_slp(slp_model_dir)
        self._load_tap(tap_model_dir)
        self._load_tlp(tlp_model_path)

    # ---------------- 加载 ----------------
    def _load_slp(self, model_dir: str) -> None:
        try:
            discovered = _discover_models(model_dir)
        except Exception as e:  # noqa: BLE001
            logger.warning("静态链接预测模型目录扫描失败: %s", e)
            discovered = []

        for mi in discovered:
            try:
                handler = SLPModelHandler(
                    device=self.device,
                    top_k=self.default_top_k,
                    relation_mapping=self.relation_mapping,
                )
                ...
                self._slp_cache[mi["name"]] = (
                    model, meta, handler.data, handler.dataset,
                )
                logger.info("✓ 静态链接预测模型已加载: %s", mi["name"])
            except Exception as e:  # noqa: BLE001
                logger.error("静态链接预测模型 '%s' 加载失败: %s", mi["name"], e)

    def _load_tap(self, model_dir: str) -> None:
        ...
        if not (os.path.exists(graph_path)
            ...
            logger.warning(
                "时序属性预测模型不完整，跳过加载（需: temporal_kg_graph.pt / "
        ...
        try:
            ...
            logger.info("✓ 时序属性预测模型已加载: 切片数=%d", total)
        except Exception as e:  # noqa: BLE001
            logger.error("时序属性预测模型加载失败: %s", e)
            self._tap_config = None

    def _load_tlp(self, model_path: "str | None") -> None:
        if not model_path:
            logger.info("未指定 --model-path，跳过 TLP 链接预测模型加载")
            return
        if not os.path.exists(model_path):
            logger.warning(
                "TLP 模型文件不存在，跳过加载: %s", model_path)
            return
        ...
        try:
            ...
            try:
                ...
                logger.warning("TLP 关系端点预构建失败（将按需构建）: %s", e)
            self._tlp_config = {"model": model, "data": data}
            logger.info("✓ TLP 链接预测模型已加载: %s", model_path)
        except Exception as e:  # noqa: BLE001
            logger.error("TLP 模型加载失败: %s", e)
            self._tlp_config = None
    ...
    def predict_slp(
        ...
        """执行静态链接预测，返回结构化 JSON dict。"""
        if not self._slp_cache:
            raise RuntimeError("未加载任何静态链接预测模型，请检查 --slp-model-dir")

        name = model_name or next(iter(self._slp_cache))
        if name not in self._slp_cache:
            raise ValueError(
                f"未知静态链接预测模型: {model_name}，可用: {list(self._slp_cache)}")

        model, meta, data, dataset = self._slp_cache[name]
    ...
    def predict_tap(self, slice_idx: "int | None" = None) -> dict:
        """对指定时序切片执行健康静态链接预测 + 油温属性预测，返回结构化 dict。"""
        if self._tap_config is None:
            raise RuntimeError(
                "未加载时序属性预测模型/图谱，请检查 --tap-model-dir")

        kg = self._tap_config["kg_graph"]
        total = self._tap_config["total"]
        ...
        diag_model = self._tap_config["diag_model"]
        tgn_model = self._tap_config["tgn_model"]
        ...
        with torch.no_grad():
            ...
            # 2) TGN 油温属性预测（输出为 z-score，需反标准化成摄氏度）
    ...
    def predict_tlp(
        ...
        """TLP 链接预测：给定 (头, 关系, 时间) 预测 Top-K 尾实体。
        ...
        if queries is not None:
            ...
            for i, q in enumerate(queries):
                try:
                    results.append(self._predict_tlp_one(
    ...
    def _predict_tlp_one(
        ...
        """TLP 链接预测单条推理。"""
        if self._tlp_config is None:
            raise RuntimeError("未加载 TLP 模型，请检查 --model-path")

        model = self._tlp_config["model"]
        data = self._tlp_config["data"]
    ...
    def health(self) -> dict:
        return {
            ...
            "slp_models": list(self._slp_cache.keys()),
            "tap_config_loaded": self._tap_config is not None,
            "tlp_config_loaded": self._tlp_config is not None,
        }
...
class _Handler(BaseHTTPRequestHandler):
    ...
    # ---------------- 各路由处理 ----------------
    def _do_slp_infer(self, payload: dict) -> None:
        """POST /slp：静态链接预测推理。"""
        ...
        result = _Handler.service.predict_slp(
            str(symptoms), model_name=model, top_k=top_k
        )
        self._send_json(result)

    def _do_tlp_infer(self, payload: dict) -> None:
        """POST /tlp：动态链接预测推理（单次或批量）。"""
        queries = payload.get("queries")
        if queries is not None:
            # 批量推理：queries 为查询列表
            try:
                result = _Handler.service.predict_tlp(queries=queries)
            except (ValueError, RuntimeError) as e:  # noqa: BLE001
    ...
    def do_POST(self):
        ...
        try:
            # 静态链接预测推理
            if path in ("/slp", "/slp/"):
                self._do_slp_infer(payload)
                return

            # 动态链接预测推理（单次或批量）
            if path in ("/tlp", "/tlp/"):
                self._do_tlp_infer(payload)
                return

            # 时序属性预测推理
            if path in ("/tap", "/tap/"):
                self._do_tap_infer(payload)
                return
...
def run_server(
    ...
    svc = InferenceService(
        slp_model_dir=slp_model_dir,
        tap_model_dir=tap_model_dir,
        tlp_model_path=tlp_model_path,
        device=device,
        top_k=top_k,
    ...
    print(f"   静态链接预测模型: {svc.health()['slp_models'] or '(无)'}")
    print(f"   时序属性预测: {'已加载' if svc._tap_config else '未加载'}")
    print(f"   TLP     : {'已加载' if svc._tlp_config else '未加载'}")
    print(f"   接口: GET /health  POST /slp  POST /tap  "
          f"POST /tlp  POST /evaluate")
    try:
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="统一推理 —— HTTP 服务（静态链接预测 + 时序属性预测 + TLP 链接预测）")
    ...
    parser.add_argument(
        "--slp-model-dir", type=str,
        default=_slp_config().model_path,
        help="静态链接预测模型目录（自动发现 .pt/.pth），默认来自 config.yaml"
    )
    parser.add_argument(
        "--tap-model-dir", type=str,
        default=_tap_config().model_path,
        help="时序属性预测模型/图谱目录（含 temporal_*.pt），默认来自 config.yaml"
    )
    parser.add_argument(
        "--model-path", type=str,
        default=_tlp_config().model_path,
        help="TLP 链接预测已训练模型 .pt 路径（可选）"
    )
    ...
    parser.add_argument(
        ...
        help="静态链接预测默认返回 Top-K 实体数，默认来自 config.yaml"
    )
    parser.add_argument(
        "--relation-mapping", type=str, default=None,
        help="静态链接预测关系映射 JSON 字符串（默认从 config.yaml 读取）"
    )
    return parser
...
def main() -> None:
    ...
    relation_mapping: "dict | None" = None
    if args.relation_mapping:
        try:
            relation_mapping = json.loads(args.relation_mapping)
        except json.JSONDecodeError:
            relation_mapping = None
    if relation_mapping is None:
        relation_mapping = _default_relation_mapping()
    run_server(
        slp_model_dir=args.slp_model_dir,
        tap_model_dir=args.tap_model_dir,
        tlp_model_path=args.model_path,
        host=args.host,
        port=args.port,
        device=args.device,
        top_k=args.top_k,
