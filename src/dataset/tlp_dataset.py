...
def iter_train_batches(data_dir,
    ...
    """
    import csv as _csv
    edgelist = os.path.join(data_dir, dynamic_edge_file)
    staticlist = os.path.join(data_dir, static_edge_file)
...
def load_tlp_data(
    ...
    参数：
        data_dir          : 数据集所在目录（绝对或相对路径）。动态边文件与
                            静态边文件均按相对该目录的路径拼接。
        dynamic_edge_file : 时序（动态）边文件名，相对 data_dir，必填。
                            文件为 CSV，含表头 ts,head,tail,relation_type，
                            每行表示一条带时间戳的时序边。
        static_edge_file  : 静态边文件名，相对 data_dir，必填。
                            文件为 CSV，含表头 head,tail,relation_type（无时间戳），
                            表示始终成立的背景关系边。
        val_start         : int，验证集起始年份（含）。ts < val_start 的边归入
                            训练集；val_start <= ts < test_start 归入验证集；
                            默认 DEFAULT_VAL_START (1998)。
        test_start        : int，测试集起始年份（含）。ts >= test_start 的边归入
                            测试集；默认 DEFAULT_TEST_START (2008)。
        static_sentinel   : int/float，静态边的「哨兵时间戳」，默认
                            DEFAULT_STATIC_SENTINEL (1899)。取小于任意时序时间戳的
                            值，使静态边在任意时序查询时都满足 tt <= qt，从而始终
                            可作为历史邻居参与聚合。
        build_train_arrays: bool，是否将训练边与静态边整体构建为内存中的
                            np.ndarray。为 True 时返回 train_quads / static_quads
                            （约 150 万条，整体驻留内存）；为 False 时二者返回空数组，
                            训练边改为 iter_train_batches 流式分批产出，以显著降低
                            峰值内存。验证/测试四元组始终构建（体积小，评测与
                            checkpoint 需要）。

    返回 dict，含：
        entity2id / id2entity : 实体字符串 <-> int id 的双向映射（首现顺序构建，
                                先扫时序边再扫静态边，确定性可复现）
        relation2id / id2relation : 关系字符串 <-> int id 的双向映射
        num_ent / num_rel : 实体数 / 关系数
        train_quads  : np.ndarray (N,4) int64 [ts, h, t, r]，训练时序边
                       （ts < val_start）；build_train_arrays=False 时为空数组
        val_quads    : np.ndarray (V,4) int64 [ts, h, t, r]，验证时序边
        test_quads   : np.ndarray (T,4) int64 [ts, h, t, r]，测试时序边
        static_quads : np.ndarray (M,4) int64 [ts(=sentinel), h, t, r]，静态边；
                       build_train_arrays=False 时为空数组
        neigh        : dict eid -> (nb np.int64, rel np.int64, tt np.float32)，
                       含反向边的邻接表，供 entity_repr 向量化聚合历史邻居
        true_tails   : dict (h, r, ts) -> set of tail ids，过滤式评测用真实尾集合
        time_scale   : float，时间归一化尺度（时间跨度/10），配合 t_center 使时间
                       信号可分辨
        t_center     : float，时间中点，供模型对时间居中
        splits       : dict，记录本次使用的 val_start / test_start / static_sentinel
    """
    edge_list = os.path.join(data_dir, dynamic_edge_file)
    static_list = os.path.join(data_dir, static_edge_file)
    ...
    with open(edge_list, encoding="utf-8") as f:
        for row in csv.DictReader(f):
            ts = int(row["ts"])
            h = eid(row["head"]);
            t = eid(row["tail"]);
            r = rid(row["relation_type"])
            temporal.append((ts, h, t, r))

    # 静态边（哨兵时间）
    static = []
    with open(static_list, encoding="utf-8") as f:
        for row in csv.DictReader(f):
            h = eid(row["head"]);
            t = eid(row["tail"]);
            r = rid(row["relation_type"])
